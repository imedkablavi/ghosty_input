...
from PySide6.QtWidgets import QApplication, QCheckBox, QComboBox, QFormLayout, QFrame, QGridLayout, QGroupBox, QHBoxLayout, QLabel, QMainWindow, QMenu, QMessageBox, QPlainTextEdit, QPushButton, QSlider, QStyle, QSystemTrayIcon, QTabWidget, QVBoxLayout, QWidget
...
class LinuxWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self.setWindowTitle('Ghosty Input · Linux Control Center')
        ...
        header.addWidget(QLabel('Linux native input · persistent cameras · local processing'))
        header.addStretch()
    ...
    def _sidebar(self) -> QWidget:
        ...
        hint = QLabel('Auto prefers Linux uinput. Camera routing uses persistent /dev/v4l aliases when available.')
    ...
    def _setup_tray(self) -> None:
        if not QSystemTrayIcon.isSystemTrayAvailable():
            return
        self.tray = QSystemTrayIcon(QApplication.style().standardIcon(QStyle.SP_ComputerIcon), self)
        self.tray.setToolTip('Ghosty Input')
        menu = QMenu(self)
    ...
    def _toggle_engine(self) -> None:
        self.stop_engine() if self.worker and self.worker.isRunning() else self.start_engine()

    def _quit(self) -> None:
        self._quitting = True
        self.stop_engine()
        self.tray.hide() if self.tray else None
        QApplication.quit()

    @staticmethod
    def _select(combo: QComboBox, value: str) -> None:
        index = combo.findData(value)
        combo.setCurrentIndex(index) if index >= 0 else None

    def populate_cameras(self) -> None:
        devices = discover_cameras()
        self._fill_camera(self.front_camera, devices, self.config.front_camera, self.config.front_camera_id)
        self._fill_camera(self.top_camera, devices, self.config.top_camera, self.config.top_camera_id)
        if hasattr(self, 'log'):
            self.log.appendPlainText(f'Camera discovery refreshed · {len(devices)} device(s).')

    @staticmethod
    def _fill_camera(combo: QComboBox, devices: list[CameraDevice], selected: int, stable_id: str) -> None:
        combo.clear()
        target = -1
        for device in devices:
            ...
            if stable_id and device.stable_id == stable_id:
                ...
                target = row
        if target < 0:
            combo.addItem(f'Camera {selected} · currently unavailable', (selected, stable_id))
    ...
    def _camera(combo: QComboBox) -> tuple[int, str]:
        ...
        return (int(data[0]), str(data[1])) if isinstance(data, (tuple, list)) and len(data) == 2 else (int(data), '')
    ...
    def _engine_ready(self, info) -> None:
        ...
        self.log.appendPlainText(f'Camera {info.index} · {info.resolution} · {info.backend} · {info.path}')
    ...
    def stop_engine(self) -> None:
        if self.worker:
            ...
            self.worker = self.worker if self.worker.isRunning() else None
        self.start.setEnabled(True)
        self.stop.setEnabled(False)
        self.calibrate.setEnabled(False)
    ...
    def _sync_desktop(self) -> None:
        self.desktop_button.setText('Remove app launcher' if desktop_entry_installed() else 'Install app launcher')
        self.autostart_button.setText('Disable autostart' if autostart_enabled() else 'Enable autostart')
    ...
    def toggle_desktop(self) -> None:
        try:
            if desktop_entry_installed():
                ...
                self.log.appendPlainText(f'Linux application launcher installed: {install_desktop_entry()}')
            self.refresh_diagnostics()
        except Exception as exc:
    ...
    def toggle_autostart(self) -> None:
        try:
            ...
            self.log.appendPlainText('Desktop-session autostart enabled.' if enabled else 'Autostart disabled.')
            self.refresh_diagnostics()
        except Exception as exc:
    ...
    def _update_metrics(self, m: RuntimeMetrics) -> None:
        self.metric_fps.setText(f'FPS {m.front_fps:.0f}')
        self.metric_res.setText(f'Camera {m.camera_resolution}')
        self.metric_hand.setText(f'Hand {m.hand_confidence * 100:.0f}%' if m.hand_count else 'Hand —')
        self.metric_backend.setText(f'Input {m.input_backend}')

    @staticmethod
    def _pixmap(frame) -> QPixmap:
        rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        h, w, channels = rgb.shape
        return QPixmap.fromImage(QImage(rgb.data, w, h, channels * w, QImage.Format_RGB888).copy())
    ...
    def closeEvent(self, event) -> None:
        if not self._quitting and self.config.linux_close_to_tray and self.tray and self.tray.isVisible():
            event.ignore()
            self.hide()
            if not self._tray_notice:
                self.tray.showMessage('Ghosty Input', 'Ghosty Input is still running. Use the tray menu to quit.', QSystemTrayIcon.MessageIcon.Information, 3500)
...
def run_linux_ui(*, start_minimized: bool=False) -> int:
    ...
    window.hide() if minimized and window.tray else window.showMinimized() if minimized else window.show()
    return app.exec()
